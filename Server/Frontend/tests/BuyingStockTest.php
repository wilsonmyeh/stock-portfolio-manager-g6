	...
	class BuyingStockTest extends PHPUnit_Framework_TestCase
	{
		...
		public function testBuyingStockThatDoesNotExistShouldResultInUnchangedPortfolio(){
		    ...
		    try{
				...
				//Assert
				$this->assertEquals($prevAccountBalance, $newAccountBalance, "pre and post sale account balances are not the same when buying new stock that doesn't exist");
		...
		public function testBuyingStockWithInsufficientFundsShouldResultInUnchangedPortfolio(){
		    ...
		    try{
				...
				//Assert
				$this->assertEquals($prevAccountBalance, $newAccountBalance, "pre and post sale account balances are incorrect when buying new stock without sifficient funds");
		...
		public function testBuyingNotPreviouslyOwnedStockWithSufficientFundsShouldCreateNewLocalOwnedStockObjectAndPushUpdatesToLocalObjectAndParse(){
		    ...
		    try{
				...
				// Assert
				$this->assertArrayNotHasKey($stockTicker, $prevStockArray, "pre-sale, previous stock array incorrectly contains the new stock when buying new stock that was not previously owned");
				$this->assertArrayHasKey($stockTicker, $newStockArray, "post-sale, new stock array incorrectly does not contain the new stock when buying new stock that was not previously owned");
		...
		public function testBuyingPreviouslyOwnedStockWithSufficientFundsShouldOnlyUpdateNumberOfShares()
		{	
			//Arrange
			//creating a variable for this test
			$portfolioObj = new Portfolio();
